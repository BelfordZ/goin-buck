...
import { Fact, FactBuffer } from '../types';
import { config } from '../config';
import { loggers } from '../utils/logger';

const logger = loggers.workingMemory;

/**
 * Concrete implementation of working memory using LRU cache behavior
 */
export class LRUWorkingMemory extends WorkingMemory {
  private factBuffer: FactBuffer;
  private factMap: Map<string, Fact>;

  constructor(
    name: string,
    capacity: number = config.memory.contextRetentionSize,
  ...
  async evictByWeight(): Promise<void> {
    logger.debug({
  ...
  async getSampleByWeight(sampleSize: number = 5): Promise<Fact[]> {
    logger.debug({
