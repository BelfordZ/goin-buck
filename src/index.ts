...
import { extractFactFromText, generateEmbedding } from './services/openai';
...
import { SupabaseEmotionalMemory } from './implementations/emotional-memory';
...
export class CognitiveSystem {
  ...
  private emotionalMemory: SupabaseEmotionalMemory;
  private sensoryMemories: Map<string, SensoryContextMemory>;
  private sleepCycle: NodeJS.Timeout | null = null;
  private insightService: InsightService;

  constructor() {
    // Initialize memory systems
    this.workingMemory = new LRUWorkingMemory(
      'active-memory',
      config.memory.contextRetentionSize,
      'lru',
      0.5
    );
    this.longTermMemory = new SupabaseLongTermMemory('long-term-memory');
    this.emotionalMemory = new SupabaseEmotionalMemory('emotional-memory');
  ...
  private initializeSensoryMemories(): void {
    const inputTypes = ['read-text', 'hear-text', 'feel-text', 'smell-text', 'taste-text'];
    for (const type of inputTypes) {
      this.sensoryMemories.set(
        type,
        new SensoryContextMemory(
          `${type}-memory`,
          type,
          config.memory.contextRetentionHours * 60 * 60 * 1000
        )
      );
    }
  }
   ...
   */
  async processTextInput(content: string): Promise<SensoryProcessingResult> {
    ...
    const recentLongTermFacts = await Promise.all(
      factIds.map(id => this.longTermMemory.retrieve(id))
    ).then(facts => facts.filter((fact): fact is Fact => fact !== null));

    // Merge contexts to get the richest context
    const context: SensoryContext = {
      type: 'read-text',
    ...
    const extractedFact = await extractFactFromText(content, context);
    ...
    const similarPatterns = await this.longTermMemory.findSimilarPatterns(
      fact.embedding,
    ...
    if (similarPatterns.length > 0) {
      await Promise.all(
        similarPatterns.map(pattern =>
          this.longTermMemory.strengthenPattern(pattern.id, fact.weight)
        )
      );
    } else if (fact.weight > config.memory.patternThreshold) {
   ...
   */
  private calculateFactWeight(impact: EmotionalQuadrant): number {
   ...
   */
  startSleepCycle(intervalHours: number = 24): void {
    if (this.sleepCycle) return;
   ...
   */
  private async processSleepCycle(): Promise<void> {
    // Get high-weight facts from short-term memory
    const shortTermSample = await this.workingMemory.getSampleByWeight(
      config.memory.sleepCycleFactCount
    );
   ...
   */
  private reduceEmotionalImpact(impact: EmotionalQuadrant): EmotionalQuadrant {
    return {
      joy: impact.joy * config.memory.emotionalDecayRate,
      calm: impact.calm * config.memory.emotionalDecayRate,
      anger: impact.anger * config.memory.emotionalDecayRate,
      sadness: impact.sadness * config.memory.emotionalDecayRate,
    };
  }
   ...
   */
  async getEmotionalState(): Promise<EmotionalQuadrant> {
   ...
   */
  async getEmotionalTrends(startTime: Date, endTime: Date): Promise<EmotionalQuadrant[]> {
  ...
  async findCrossContextPatterns(
    ...
    const patterns = await this.longTermMemory.getSignificantPatterns(
      config.memory.sleepCycleFactCount
    );

    // Group patterns by emotional signature similarity
    const groupedPatterns = new Map<string, Pattern[]>();
    ...
    const crossContextPatterns: CrossContextPattern[] = [];
   ...
   */
  private getEmotionalSignatureKey(signature: EmotionalQuadrant): string {
    const threshold = config.emotional.neutralThreshold;
    return [
