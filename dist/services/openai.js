...
exports.extractFactFromText = extractFactFromText;
exports.analyzeEmotionalQuadrant = analyzeEmotionalQuadrant;
exports.generateEmbedding = generateEmbedding;
exports.generateText = generateText;
const openai_1 = __importDefault(require("openai"));
const config_1 = require("../config");
const logger_1 = require("../utils/logger");
const logger = logger_1.loggers.openai;
logger.debug(`OpenAI API key: ${config_1.config.openai.apiKey ? '(set)' : '(not set)'}`);
const openai = new openai_1.default({
    apiKey: config_1.config.openai.apiKey,
});
function logRequest(type, messages, model) {
    logger.debug({
        event: 'request',
        type,
        model,
...
function logResponse(type, response, parsed) {
    logger.debug({
        event: 'response',
        type,
...
async function extractFactFromText(text, context) {
    const contextPrompt = context ? `
    Previous context:
    ${context.recentFacts.map((fact) => fact.content).join('\n')}
  ` : '';
    logger.info({
...
Example: {"content": "The sun is shining", "emotionalImpact": {"joy": 0.8, "calm": 0.5, "anger": -0.2, "sadness": -0.3}}`
        ...
        logRequest('Fact Extraction', messages, config_1.config.openai.model);
...
async function analyzeEmotionalQuadrant(content) {
    logger.info({
...
Example: {"joy": 0.8, "calm": 0.5, "anger": -0.2, "sadness": -0.3}`
        ...
        logRequest('Emotion Analysis', messages, config_1.config.openai.model);
        const response = await openai.chat.completions.create({
            model: config_1.config.openai.model,
            messages,
            temperature: 0.3,
...
async function generateText(prompt, model = config_1.config.openai.model) {
    logger.info({
